...
type Puzzle struct {
	...
	numFreeInRow [utils.SideLen]uint8
	numFreeInCol [utils.SideLen]uint8
	numFreeInBox [utils.SideLen]uint8
}
...
func newPuzzle() Puzzle {
	...
	for i := 0; i < utils.SideLen; i++ {
		pzl.numFreeInRow[i] = utils.SideLen
		pzl.numFreeInCol[i] = utils.SideLen
		pzl.numFreeInBox[i] = utils.SideLen
	}
...
func ReadSudoku(str string) (p Puzzle, err error) {
	...
	for i := 0; i < len(str); i++ {
		...
		if wasPlaced, err := pzl.place(row, col, box, utils.Entry(entry)); err != nil || !wasPlaced {
...
func (p *Puzzle) getLocationAndEntries() (row, col, box int, entries []utils.Entry, err error) {
...
func (p *Puzzle) getEntries(row, col, box int) ([]utils.Entry, error) {
	//return p.stupidGetEntries(row, col, box)
	return p.quickGetEntries(row, col, box)
}
func (p *Puzzle) stupidGetEntries(row, col, box int) ([]utils.Entry, error) {
...
func (p *Puzzle) quickGetEntries(row, col, box int) ([]utils.Entry, error) {
...
func (p *Puzzle) getBestCol(bestRowIndex int, bkCol, bkBox *utils.BestKnown) {
	bkCol.Reset()

	for c := range p.numFreeInCol {
		if p.tiles[bestRowIndex][c] != utils.EmptyTile {
			continue
		}
...
func (p *Puzzle) getBestRow(bestColIndex int, bkRow, bkBox *utils.BestKnown) {
	bkRow.Reset()

	for r := range p.numFreeInRow {
		if p.tiles[r][bestColIndex] != utils.EmptyTile {
			continue
		}
...
func (p *Puzzle) place(row, col, box int, entry utils.Entry) (bool, error) {
	numPlacements++
...
func (p *Puzzle) entryIsPresent(row, col, box int, ePresence utils.Presence) (error) {
	return p.entryIsPresentQuickly(row, col, box, ePresence)
	//return p.entryIsPresentSlowly(row, col, box, ePresence)
}
func (p *Puzzle) entryIsPresentSlowly(row, col, box int, ePresence utils.Presence) (error) {
...
func (p *Puzzle) entryIsPresentQuickly(row, col, box int, ePresence utils.Presence) (error) {
...
func (p *Puzzle) solve() (solution *Puzzle, err error) {
	...
	for _, entry := range entries {
		...
		if wasPlaced {
			if pClone.numPlaced == utils.NumTiles {
				return pClone, nil
			}
